...
import BalanceTree from './shares/balance-tree'

import Distributor from '../build/MerkleDistributor.json'
import TestERC20 from '../build/TestERC20.json'
import { parseBalanceMap } from './shares/parse-balance-map'
...
const TEST_ROOT32 = '0x1234000000000000000000000000000000000000000000000000000000005678'
...
describe('MerkleDistributor', () => {
  ...
  describe('#token', () => {
    it('returns the token address', async () => {
      const distributor = await deployContract(wallet0, Distributor, [token.address, TEST_ROOT32], overrides)
  ...
  describe('#merkleRoot', () => {
    it('returns the zero merkle root', async () => {
      const distributor = await deployContract(wallet0, Distributor, [token.address, TEST_ROOT32], overrides)
      expect(await distributor.merkleRoot()).to.eq(TEST_ROOT32)
    })
  })
  ...
  describe('#claim', () => {
    ...
    describe('two account tree', () => {
      ...
      it('gas', async () => {
        ...
        expect(receipt.gasUsed).to.eq(78263)    //78466
      })
    })
    describe('larger tree', () => {
      ...
      it('gas', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(80757)        // 80960
      })
      ...
      it('gas second down about 15k', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(65737)        //65940
      })
    })
    ...
    describe('realistic size tree', () => {
      ...
      it('gas', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(91447)     //91650
      })
      it('gas deeper node', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(91383)    //91586
      })
      it('gas average random distribution', async () => {
        ...
        expect(average).to.eq(76872)          //77075
      })
      ...
      it('gas average first 25', async () => {
        ...
        const average = total.div(count)
        expect(average).to.eq(62621)        //62824
      })
